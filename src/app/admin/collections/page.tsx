...
const EMPTY: Collection = { id: "", name: "", description: "" };
...
export default function AdminCollectionsPage() {
  ...
  function validate(isEdit: boolean): Record<string, string> {
    ...
    if (!isEdit) {
      ...
      else if (collections.some((c) => c.id === form.id)) errs.id = "ID already exists.";
    }
  ...
  async function update() {
    ...
    try {
      ...
      const res = await fetch(`/api/collections/${editingId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(rest) });
      if (res.ok) { cancelEdit(); await load(); }
  ...
  return (
    <>
      <AdminPageHeader
        title="Collections"
        description="Manage stay collections shown as filter options."
        actions={!showForm && <Button onClick={() => setShowForm(true)}>+ New Collection</Button>}
      />
            ...
            <div className="bg-white rounded-2xl border border-neutral-200/80 p-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <Label htmlFor="col-id" className="text-xs font-bold uppercase tracking-wide text-neutral-700">ID (slug)</Label>
                  <Input
                    id="col-id"
                ...
                <div className="space-y-1">
                  <Label htmlFor="col-name" className="text-xs font-bold uppercase tracking-wide text-neutral-700">Name</Label>
                  <Input
                    id="col-name"
                    placeholder="e.g., Luxury Villas"
                    value={form.name}
                ...
                <div className="space-y-1">
                  <Label htmlFor="col-desc" className="text-xs font-bold uppercase tracking-wide text-neutral-700">Description (optional)</Label>
                  <Input
                    id="col-desc"
                ...
                <div className="flex gap-3 pt-2">
                  <Button variant="black" onClick={editingId ? update : create} disabled={submitting} className="flex-1">
                    {submitting ? "Saving…" : editingId ? "Update" : "Add Collection"}
                  </Button>
        ...
        <div className={showForm ? "lg:col-span-8" : "lg:col-span-12"}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <p className="text-neutral-400 font-bricolage text-sm">No collections yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {collections.map((c) => (
                <div key={c.id} className="bg-white rounded-2xl border border-neutral-200/80 p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-neutral-900 font-bricolage">{c.name}</span>
                        <span className="text-[11px] text-neutral-400 font-bricolage bg-neutral-100 px-2 py-0.5 rounded-full">{c.id}</span>
                      </div>
                      {c.description && (
                        <p className="mt-1 text-sm text-neutral-500 font-bricolage">{c.description}</p>
                      )}
                    </div>
